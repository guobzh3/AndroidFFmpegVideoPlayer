...
//}
public class TextureView_filterEngine  {
...
//    private int mTextureDataHandle;
    ...
    private int u_Texture_location; // 句柄
    ...
    // 对应了SR和原图的两个外部纹理
    public int mOESTextureId_1 = -1;
    public int mOESTextureId_2 = -1;
...
//    private int texture2_id;
    ...
    public TextureView_filterEngine(Context mActivityContext,int OESTextureId_1 , int OESTextureId_2) {
        this.mActivityContext = mActivityContext;
        this.mOESTextureId_1 = OESTextureId_1;
        this.mOESTextureId_2 = OESTextureId_2;
        // 初始化顶点数据
...
//        texture2_id = TextureHelper.loadTexture(mActivityContext , R.drawable.usb_android);
    ...
    public void drawframe( ) {
            ...
            u_Texture_location = GLES20.glGetUniformLocation(program_handle, "u_Texture");

            Log.i(gl_tag, "locations: " + program_handle + " " + mPosition_handle + " " + mcord_handle + " " + u_Texture_location); // 3 0 1 -1 0 -1

            // Set the active texture unit to texture unit 0.
            GLES20.glActiveTexture(GLES20.GL_TEXTURE0);
            GLUtils.texImage2D(GLES20.GL_TEXTURE_2D, 0, test_bitmap, 0); // 要bitmap绑定到纹理上（如果设置为外部纹理，就可以在外面绑定）
            // Bind the texture to this unit. 绑定纹理（这个在前面也绑定过了）
            // 应该是每个纹理有一个纹理id，这里绑定那一个就说明使用的是哪一个
            GLES20.glBindTexture(GLES20.GL_TEXTURE_2D, this.mOESTextureId_1);
//            GLES20.glBindTexture(GLES11Ext.GL_TEXTURE_EXTERNAL_OES, this.mOESTextureId_1);
            ...
            // 画第二个图
            GLES20.glActiveTexture(GLES20.GL_TEXTURE0);
            GLUtils.texImage2D(GLES20.GL_TEXTURE_2D, 0, this.test_bitmap, 0);
            GLES20.glBindTexture(GLES20.GL_TEXTURE_2D, this.mOESTextureId_2);
//            GLES20.glBindTexture(GLES11Ext.GL_TEXTURE_EXTERNAL_OES, this.mOESTextureId_2);
...
//            readBufferPixelToBitmap(this.width,this.height); // 保存图片下来看看
